...
function submitOrder() {
  ...
  for (const id in cart) {
    const item = cart[id];
    cartText += `• ${item.name} — Размер: ${item.size} — ${item.price} сом x ${item.quantity} = ${item.price * item.quantity} сом\n`;
  }
  cartText += `\nОбщая сумма со скидкой: ${calculateTotal()} сом`;
...
function handleFormSubmit() {
  ...
  for (const id in cart) {
    const item = cart[id];
    const itemTotal = item.price * item.quantity;
    cartText += `• ${item.name} — Размер: ${item.size} — ${item.price} сом x ${item.quantity} = ${itemTotal} сом\n`;
    total += itemTotal;
  }
  ...
  cartText += `\nОбщая сумма со скидкой: ${(total * 0.8).toFixed(2)} сом`;
